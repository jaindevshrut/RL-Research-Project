...
(Method~2) following~\cite{dabney2018qrdqn}, in which the dueling V-head and
...
so that the design trade-off is visible to the reader. We end with concrete
...
trade-off empirically by building a parallel QR-DQN backbone (Method~2,
Section~\ref{sec:method2}) and comparing it head-to-head with our
...
\subsection{Dueling Double DQN with Bootstrap Ensemble}
...
\subsection{Method~2: Quantile-Regression DQN Backbone}
\label{sec:method2}
...
\textbf{Method~2}, on top of Quantile-Regression DQN
...
result? Method~2 keeps every other element identical: the $7\times 7$
...
The appraisal extractor consumes Method~2's outputs through three
...
statistics that Method~1 reads from a representation that QR-DQN does
not provide.
...
\subsection{Method~2 Training Objective: Asymmetric Quantile Huber Loss}
\label{sec:qhuber}
Method~2 (Section~\ref{sec:method2}) replaces the scalar Huber loss above
...
schedule are identical to Method~1, so the only intentional source of
...
Random seed                     & 0 \\
\midrule
\multicolumn{2}{l}{\emph{Method~2 only (QR-DQN)}} \\
Quantile atoms $N$              & 51 \\
...
\texttt{extended\_8dim} (full vector on the Method~1 backbone), and
\texttt{qrdqn\_8dim} (full vector on the Method~2 backbone). All three
...
Section~\ref{sec:method2}. The Method~2 entry-point is
\texttt{src/train\_qrdqn.py}, separate from the Method~1 entry-point
...
(C2)~\textbf{Extended-8D (Method~1)}: the full eight-dimensional vector on
the Dueling Double DQN with bootstrap ensemble; this is the headline
configuration of the paper.
(C3)~\textbf{Extended-8D (Method~2)}: the same eight-dimensional vector
on the QR-DQN backbone of Section~\ref{sec:method2}, which uses $N{=}51$
...
not competitive: we report it to expose whether Method~1's gain is
...
informational gain comes from the four added channels. Method~1
...
Method~2 (green). Method~2 attains a competitive score on its own and
...
Method~1 (Section~\ref{sec:results}-A); Method~2's greedy return
...
\subsection{Comparison Against Method~2 (QR-DQN)}
\label{sec:method2_results}
Method~2 (Section~\ref{sec:method2}) is run under the same seed, the
same total-frame budget and the same eight-dimensional appraisal vector
as Method~1, with only the value head and the Bellman loss replaced by
...
of Method~1 are doing real work, or whether any value-head
representation that exposes a usable uncertainty signal would have
sufficed. Method~2 attains classification accuracy $0.868$, paper-style
...
whereas the cross-head dispersion $\sigma_{K}(s)$ of Method~1 retains
structured epistemic content even after the value function converges.
Second, Method~2's substitute for the dueling V-head,
...
that Method~1's separate V-head explicitly avoids. The greedy-policy
...
Method~2 to close the gap: the question we set out to answer is whether
...
modelling oversight. Table~\ref{tab:master} consolidates the headline
...
Dueling Double DQN backbone of Method~1; Exp~3 swaps in the QR-DQN
backbone of Method~2 (Section~\ref{sec:method2}). Reported in the style
of Table~V of~\cite{zhang2023modeling}.}
\label{tab:master}
...
Exp 2 & Extended-8D & Method~1 & 8 & \textbf{0.903} & \textbf{0.757} & \textbf{0.214} \\
